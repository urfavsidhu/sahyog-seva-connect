...
function ChatPage() {
  ...
  useEffect(() => {
    // On mobile, land on the conversation list first so every chatted-with
    // worker is visible; only auto-open a thread in the desktop split view.
    if (isDesktop && conversations.length && !activeId) setActiveId(conversations[0]!.workerId);
  }, [conversations, activeId, isDesktop]);
